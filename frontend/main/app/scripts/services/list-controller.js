...
angular.module('main').factory('listController', function($log, $q, localStorageService, $location, $timeout) {
    ...
    return function(oMainConfig) {
        ...
        //On After save item callback
        var _onAfterSave = oMainConfig.afterSave;
        if (!_onAfterSave || typeof _onAfterSave != 'function') {
            _onAfterSave = function() {};
        }

        //On After Submit item callback
        var _onAfterSubmit = oMainConfig.afterSubmit;
        if (!_onAfterSubmit || typeof _onAfterSubmit != 'function') {
        ...
        var sCustomMethod = oMainConfig.CustomMethod;
        ...
        scope.removeItem = function(oEntity) {
            alertify.confirm('Do you really want to delete a: ' + oMainConfig.entityName + '?', function() {
                scope.$apply(function() {
                    _baseService.removeEntity(oEntity, scope.baseList).then(
                        function(data) {
        ...
        scope.deleteSelected = function() {
            if (scope.selectedCount > 0) {
                alertify.confirm('Do you really want to remove all selected items?', function() {
                    scope.$apply(function() {
        ...
        listCtrl.save = function() {
            ...
            _saveChildren().then(function() {
                if (scope.baseList.editMode) {
                    _baseService.updateBatch(scope.baseList).then(
                        function(data) {
                            deferred.resolve();
                        },
                        function() {
        ...
        var _updateList = function() {
            ...
            if (sCustomMethod) {
                return _baseService.customGet(sCustomMethod, queryParameters).then(function(data) {
                    _afterLoad(data);
                    scope.isLoading = false;
                });
            } else {
        ...
        scope.onSubmit = function() {
            scope.saveItem(scope.baseEntity).then(data => {
                _onAfterSubmit(data);
            });
        };
        ...
        listCtrl.setRotationFocus = function() {
            $timeout(function() {
                $('input, md-checkbox').off('keydown');
